...
- [`invoice-fetch`](invoice-fetch/README.md)：登录任意邮箱（新浪/163/QQ/Gmail/Outlook 等）抓取收件夹里的电子发票——附件直接下载，正文链接形式的发票会点开链接下载 PDF；只下载 PDF/图片（跳过 OFD/XML），自动去重、按"金额-项目-抬头-发票号"重命名，生成带总金额的发票清单 Excel 和四拼一汇总 PDF，可直接用于报销。

### invoice-fetch 技能介绍

`invoice-fetch` 用于一键整理邮箱发票，适合每月报销前批量收集电子发票。它会：

- 按你给的邮箱打开对应登录页（任意邮箱均可），在专用窗口登录，不碰你日常浏览器；
- 扫描收件夹最近 31 天的发票邮件，下载附件发票；邮件正文只有链接的，自动点开链接下载 PDF 发票；
- 按文件内容去重，把开票二维码小票单独放到"需要二次扫码"目录并提醒失效日期；
- 输出规范命名的发票文件、`发票清单.xlsx`（含总金额）和 `发票_四拼一汇总.pdf`。

安装与使用详见 [invoice-fetch/README.md](invoice-fetch/README.md)，最简单的方式是把里面那段话直接发给 agent。
