...
- [`invoice-fetch`](invoice-fetch/README.md)：登录新浪邮箱抓取收件夹里的电子发票（只下载 PDF/图片，跳过 OFD/XML），自动去重、按"金额-项目-抬头-发票号"重命名，并生成带总金额的发票清单 Excel 和四拼一汇总 PDF，可直接用于报销。

### invoice-fetch 技能介绍

`invoice-fetch` 用于一键整理邮箱发票，适合每月报销前批量收集电子发票。它会：

- 打开一个专用浏览器窗口让你登录新浪邮箱（登录态保留，不碰你日常浏览器）；
- 扫描收件夹最近 30 天的发票邮件，下载附件发票，也会自动点击邮件正文里的税务平台交付链接下载 PDF；
